...
func runDiff(c *ufcli.Context) error {
	...
	for _, clusterPath := range clusters {
		state, err := ParseClusterDir(cfg, infraPath, clusterPath)
		if err != nil {
	...
	for _, dr := range results {
		...
		if step == 3 && len(dr.EksTFBlocksNeeded) > 0 && len(cfg.Steps.Step3.EksTFBlocks) > 0 {
			...
			for _, blockName := range dr.EksTFBlocksNeeded {
				if block, ok := cfg.Steps.Step3.EksTFBlocks[blockName]; ok && block.Content != "" {
					fmt.Printf("  %s:\n%s\n", blockName, indentBlock(block.Content))
				}
			}
			fmt.Println()
		} else {
