...
type DiffResult struct {
	...
	StencilManaged        bool                   `json:"stencil_managed"`
	...
	EksTFBlocksNeeded     []string               `json:"eks_tf_blocks_needed,omitempty"`     // Step 3: block names to add
	LocalTFBottlerocket   bool                   `json:"local_tf_bottlerocket,omitempty"`    // Step 2: needs platform->ami_type
}
...
func ComputeDiff(cfg *Config, state *ClusterState) *DiffResult {
	dr := &DiffResult{
		...
		Path:           state.Path,
		StencilManaged: cfg.IsStencilCluster(state.ClusterName),
	...
	// AWS provider - allow flexible matching (>= 5.83 vs ~> 5.40)
...
// providerVersionMatches does a loose check (exact match or both look like 5.x).
func providerVersionMatches(current, target string) bool {
	if current == target {
		return true
	}
	// Extract major.minor for rough comparison
...
func FormatDiff(dr *DiffResult) string {
	...
	if dr.StencilManaged {
		b.WriteString("  [Stencil-managed]\n")
	}
